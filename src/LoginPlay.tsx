...
export function LoginPlay({children}:{children:ReactNode}) {
  ...
  const tune=(value:number)=>{setAwake(true);commitAngle(Math.max(HOME_ANGLE,Math.min(270,value)));};
  ...
  useEffect(()=>{if(awake&&!returning&&!locked&&Math.abs(angle-target)<=6)setLocked(true);},[angle,awake,locked,returning,target]);
  useEffect(()=>{if(!locked)return;const timer=setTimeout(returnHome,2800);return()=>clearTimeout(timer);},[locked,returnHome]);
