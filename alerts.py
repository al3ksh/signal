    ...
    def evaluate(self, data):
        with self.lock:
            ...
            if docker_ok:
                ...
                ignored = label_ignored | self.muted
                for container in data['containers']:
                    name = container['name']
                    container['monitoringMuted'] = name in ignored
                    container['monitoringMuteSource'] = 'label' if name in label_ignored else ('manual' if name in self.muted else None)
                self.watched.update(c['name'] for c in data['containers'] if c['state'] == 'running' and c['name'] not in ignored)
